...
function playerLink(name) {
  return `<button class="player-link" type="button" data-player="${name}">${name}</button>`;
}
        ...
        ${rows
          .map(
            (r, i) => `
              <tr>
                <td>${i + 1}.</td>
                <td><strong>${playerLink(r.name)}</strong></td>
                <td>${r.count}</td>
              </tr>
            `
          )
...
function getPlayerStats(name) {
  ...
  const boeh = played.flatMap((m) => m.boehmaend || []).filter((b) => b.navn === name);
  const beer = played.flatMap((m) => m.oel || []).filter((p) => p === name);

  const gkEntries = played.flatMap((m) =>
    (m.maalmaend || [])
  ...
  return {
    ...
    boeh: boeh.length,
    beer: beer.length,
    goalkeeperGames: gkEntries.length,
    goalsAgainst: gkEntries.reduce((sum, g) => sum + Number(g.maalImod || 0), 0),
    cleanSheets: gkEntries.filter((g) => g.cleanSheet).length,
    boehNotes: boeh,
    latestGoals: played
...
function ensurePlayerModal() {
  ...
  modal.addEventListener("click", (event) => {
    if (event.target.hasAttribute("data-close-player-modal")) {
      closePlayerModal();
    }
  });
...
function openPlayerModal(name) {
  ...
  const avgGoals = s.matches ? (s.goals / s.matches).toLocaleString("da-DK", { maximumFractionDigits: 2 }) : "0";
  const gkAvg = s.goalkeeperGames ? (s.goalsAgainst / s.goalkeeperGames).toLocaleString("da-DK", { maximumFractionDigits: 2 }) : "0";

  document.getElementById("playerModalContent").innerHTML = `
    <div class="eyebrow">Spillerkort</div>
    <h2 style="margin-top:.4rem">${s.name}</h2>
    ...
    ${
      s.latestGoals.length
        ? s.latestGoals
            .map((m) => `<p class="small">⚽ ${matchName(m)} · ${m.phase || "Forår"}</p>`)
            .join("")
      ...
      <tbody>
        ${gkRows
          .map(
            (g) => `
              <tr>
                <td><strong>${playerLink(g.name)}</strong></td>
                <td>${g.games}</td>
  ...
  document.getElementById("boehmandLog").innerHTML = log.length
    ? log
        .map(
          (b) => `
            <div class="log-item">
              <strong>${playerLink(b.navn)}</strong> · ${matchName(b.match)} · ${b.phase}<br>
